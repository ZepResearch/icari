...
export async function POST(request) {
  try {
    ...
    const record = await pb.collection("ICAIR_journal_form_submission").create(pbFormData)
    ...
    await resend.emails.send({
      from: "ICARI|Journal-Submission <info@robotechsummit.com>",
      to: data.email,
      subject:
        "Journal Submission Confirmation - ICARI ",
      html: getUserEmailTemplate(data),
    })

    // Send notification email to admin
    await resend.emails.send({
      from: "ICARI | Submission <info@robotechsummit.com>",
      to: "info@robotechsummit.com", // Replace with actual admin email
      subject: "New Journal Submission - from ICARI",
